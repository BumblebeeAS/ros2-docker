...
# This script requires the `uav_description` package to be built and the `bb_worlds`
...
PX4_AUTOPILOT_MODEL_DIR=${PX4_AUTOPILOT_GZ_DIR}/models/uav
PX4_AUTOPILOT_AIRFRAMES_DIR=${PX4_AUTOPILOT_DIR}/ROMFS/px4fmu_common/init.d-posix/airframes/
UAV_DESC_URDF_DIR=${UAV_SIM_DIR}/install/uav_description/share/uav_description/urdf
ETC_DIR=${UAV_SIM_DIR}/src/bring-up/etc/uav_sim
WORLDS_DIR=${UAV_SIM_DIR}/src/bb_worlds/worlds

mkdir -p "${PX4_AUTOPILOT_MODEL_DIR}"
gz sdf -p "${UAV_DESC_URDF_DIR}/uav.urdf" > "${PX4_AUTOPILOT_MODEL_DIR}/model.sdf"
ln -sf "$(realpath "${ETC_DIR}/model.config")" "${PX4_AUTOPILOT_MODEL_DIR}"
ln -sf "$(realpath "${ETC_DIR}/5000_gz_uav")" "${PX4_AUTOPILOT_AIRFRAMES_DIR}"
ln -sf "$(realpath "${WORLDS_DIR}/robotx_2024_aruco.sdf")" "${PX4_AUTOPILOT_GZ_DIR}/worlds"

if ! grep -q '5000_gz_uav' "${PX4_AUTOPILOT_AIRFRAMES_DIR}/CMakeLists.txt"; then
    sed -i '/4021_gz_x500_flow/a \ \t5000_gz_uav' \
